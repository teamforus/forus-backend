...
class ValidateImplementationPageBlocksRequest extends BaseFormRequest
{
     ...
     * @return (array|string)[]
     *
     * @psalm-return array{'blocks.*': 'nullable|array', 'blocks.*.label': 'nullable|string|max:200', 'blocks.*.title': 'required|string|max:200', 'blocks.*.description': 'required|string|max:5000', 'blocks.*.button_enabled': 'nullable|boolean', 'blocks.*.button_text': 'nullable|required_if:blocks.*.button_enabled,true|string|max:200', 'blocks.*.button_link': 'nullable|required_if:blocks.*.button_enabled,true|string|max:200', 'blocks.*.button_link_label': 'nullable|required_if:blocks.*.button_enabled,true|string|max:500', 'blocks.*.button_target_blank': 'nullable|required_if:blocks.*.button_enabled,true|boolean', 'blocks.*.media_uid.*': array}
     */
     ...
     *
     * @psalm-return list{'nullable', 'string', 'exists:media,uid', MediaUidRule}
     */
     ...
     * @return string[]
     *
     * @psalm-return array{'blocks.*.title.required': 'Het title veld is verplicht', 'blocks.*.description.required': 'Het description veld is verplicht', 'blocks.*.button_text.required_if': 'Het button text veld is verplicht', 'blocks.*.button_link.required_if': 'Het button link veld is verplicht', 'blocks.*.button_link_label.required_if': 'Het button link label veld is verplicht'}
     */
